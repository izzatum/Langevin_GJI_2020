%% Setup Density
rng('default');
load('numexAppB_mala.mat','Xmala','dksdmala')
...
ns = 2;               % Number of MCMC methods
x0 = zeros(2,1);      % Initial point
tau = [2.6e-2, 2.6e-1, 2.6];         % Step-size
...
[~,num_samples,~] = size(X);
nx = num_samples;
dksd_ula = zeros(nx,length(tau)+1);
dksd_mala = zeros(nx,length(tau)+1);
C = inv(P);
...
tic
dksd_ula(:,4) = compute_ksd(Xi, Gi, nx, "sclmed", "vanilla");
toc;

dksd_mala(:,4) = dksd_ula(:,4);
...
axis tight;
legend('\tau_{1} = 2.6 \times 10^{-2}','\tau_{2} = 2.6 \times 10^{-1}',...
       'FontSize',9);
...
figure;
subplot(1,2,1)
loglog(1:nx,dksd_ula(:,[1,2,4]),'-','LineWidth',2); hold on;
loglog(1:nx,10./sqrt(1:nx),'k-','LineWidth',2); hold off;
axis square tight;
%legend('\tau_{1} = 2.6 \times 10^{-2}','\tau_{2} = 2.6 \times 10^{-1}',...
%       'i.i.d', 'Ref: 1/\surd{N}','FontSize',11);
legend('\tau_{1}','\tau_{2}',...
       'i.i.d', 'Ref: 1/\surd{N}','FontSize',11,'Location','southwest');
legend('boxoff');
...
subplot(1,2,2)
loglog(1:nx,dksd_mala(:,[1,2,4]),'-','LineWidth',2); hold on;
loglog(1:nx,10./sqrt(1:nx),'k-','LineWidth',2); hold off;
axis square tight;
%legend('\tau_{1} = 2.6 \times 10^{-2}','\tau_{2} = 2.6 \times 10^{-1}',...
%       'i.i.d', 'Ref: 1/\surd{N}','FontSize',11);
...
xlabel('m_{2}');
ylabel('m_{1}');
title('MALA - \tau_{1} = 2.6 \times 10^{-2}');
...
xlabel('m_{2}');
ylabel('m_{1}');
title('MALA - \tau_{2} = 2.6 \times 10^{-1}');
