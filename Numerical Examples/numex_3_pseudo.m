...
tau = 0.0052;%0.0075;%1/S(1);                        % Step-length
...
subplot(223)
imagesc(reshape(xmap,n,n),[0, 1]); hold on;
plot(25,15,'marker','x','MarkerSize',10,'Color','black','LineWidth',2); hold on;
plot(25,25,'marker','x','MarkerSize',10,'Color','black','LineWidth',2); hold off;
colorbar;
axis equal tight;
xlim([1,n])
ylim([1,n])
...
subplot(224)
imagesc(reshape(sqrt(diag(Hi))./max(sqrt(diag(Hi))),n,n));
colorbar;
axis equal tight;
xlim([1,n])
ylim([1,n])
...
[X(:,:,1),G(:,:,1),tt] = samplerPrecondLipULA(funh,N,xmap,tau,diag(diag(H)),length(xmap)^(-1/3));

% Lipschitz - MALA
%[X(:,:,2),G(:,:,2)] = samplerSGLD(funh,N,xmap,tau);
%[X(:,:,2),G(:,:,2),acclip] = samplerLipMALA(funh,N,xmap,tau);
[X(:,:,2),G(:,:,2),acclip,tt2] = samplerPrecondLipMALA(funh,N,xmap,tau,diag(diag(H)),length(xmap)^(-1/3));

% MALA
%[X(:,:,3),G(:,:,3),accmala] = samplerMALA(funh,N,xmap,tau);
[X(:,:,3),G(:,:,3),accmala] = samplerPrecondMALA(funh,N,xmap,tau,diag(diag(H)));
...
% Compute ACF
acf1 = zeros(50000,ns);
acf1(:,1) = autocorr(X(1215,:,1),'NumLags',49999);
acf1(:,2) = autocorr(X(1215,:,2),'NumLags',49999);
acf1(:,3) = autocorr(X(1215,:,3),'NumLags',49999);

acf2 = zeros(50000,ns);
acf2(:,1) = autocorr(X(1225,:,1),'NumLags',49999);
acf2(:,2) = autocorr(X(1225,:,2),'NumLags',49999);
acf2(:,3) = autocorr(X(1225,:,3),'NumLags',49999);
...
plot(1:50000,zeros(50000,1),'k--','LineWidth',2); hold off;
axis tight;
...
subplot(2,2,[3,4])
loglog(1:1000,dksd,'-','LineWidth',1.5); hold on;
loglog(1:1000,595.66./sqrt(1:1000),'k-','LineWidth',2); hold off;
%axis equal;
legend('Lip-ULA','Lip-MALA','MALA','iid','Ref: 1/\surd{N}');
title('KSD','FontSize', 16)
xlabel('Number of samples, N','FontSize', 16)
ylabel('Discrepancy','FontSize', 16)
