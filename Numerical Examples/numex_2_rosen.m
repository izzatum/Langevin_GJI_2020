...
x0 = mu;%zeros(2,1);              % Initial point
tau = 3.61e-2;         % Step-size
X = zeros(2,N,ns);    % Samples
G = zeros(2,N,ns);    % Gradients of log-normal distribution
P = diag(1./sigma);

% Lipschitz - ULA
[X(:,:,1),G(:,:,1),tt1] = samplerLipULA(funh,N,x0,tau,1,2^(-1/3));
...
for i = 1:ns
    tic
    dksd(:,i) = compute_ksd(X(:,:,i)', G(:,:,i)', nx, "sclmed", "choice", diag(sigma));
    toc;
end
...
axis tight;
legend('Lip-ULA','Lip-MALA','MALA','Ref: 1/\surd{N}','FontSize',11);
